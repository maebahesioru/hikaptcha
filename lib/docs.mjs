...
const STYLE = `
  ...
  * { box-sizing: border-box; }
  ...
  nav.toc li.l3 { margin: 0 0 8px 10px; padding-left: 10px; border-left: 1px solid #2e2e33; font-size: 13px; }
  /* 折り返したときに行頭が揃うようにする(長い見出しで境界が分からなくなるのを防ぐ) */
  nav.toc li > a { text-indent: -1em; padding-left: 1em; line-height: 1.5; }
  nav.toc li.l3 > a { color: #8f8f9a; }
  nav.toc a { text-decoration: none; display: block; }
  nav.toc a:hover { color: #7dd3fc; }

  main { min-width: 0; }
  /* 本文の行長を制限する。幅いっぱいだと1行62字になり読みづらい(実測)。
     表とコードは情報量が多いので全幅のままにし、文章だけ絞る。 */
  main p, main ul, main ol, main blockquote, main h1, main h2, main h3, main h4,
  main .hint, main .layers-note { max-width: 44em; }
  ...
  /* 1列目の「短いラベル」だけ折り返さない(「1. 画像認証」等)。
     長い文章が入る表まで nowrap にすると表が横にはみ出すため、
     レンダラ側で文字数を見て .nw を付ける(実測: 一律nowrapだと1440pxでも193pxはみ出した)。 */
  td.nw { white-space: nowrap; }
  @media (max-width: 640px) { td.nw { white-space: normal; } }
...
<title>HIKAPTCHA ドキュメント</title>
<style>${STYLE}</style>
</head>
<body>
<div class="top"><div class="inner">
  <span class="title">🤖 HIKAPTCHA</span>
  <a href="#top">ドキュメント</a>
  <span class="sp"></span>
  <a class="demo" href="/">動作デモ</a>
  <a href="https://github.com/maebahesioru/hikaptcha" target="_blank" rel="noopener">GitHub</a>
  <a href="https://hikamers.app/donate" target="_blank" rel="noopener">寄付</a>
</div></div>
