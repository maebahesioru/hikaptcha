...
| 3. 画像改変 | 配信時に 3:2クロップ(構図ズラし)+反転+微小回転+ガンマ+再圧縮 | バイト一致・素朴なpHash索引 | バイト一致は完全回避(適応的攻撃への耐性は限界 → 下記参照) |
...
正方形タイル+contain では、横長画像(多数派)の上下40%が余白になり、
被写体がタイル面積の **46%**(181x107px)しか使えていなかった。

→ 配信画像を **3:2にクロップしてタイルも3:2** にし、コンテナを720pxへ拡大。
被写体は **226x151px・タイル被覆率100%**(面積で約1.8倍)になった。
